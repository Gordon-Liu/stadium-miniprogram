	...
	methods: {
		_init: function () {
			...
			if (this.data.steps == 1 &&
				...
				sourceData = getSelectOptions(this.data.sourceDataStr);
				this.setData({
					sourceData
				});
			}
			...
			if (this.data.steps == 1) {
				if (sourceData.length > 0 && isDefined(sourceData[0]['label'])) {
					labelKey = 'label';
				}
				idx = this.data.index;
			} else if (this.data.steps > 1) {
				if (sourceData.length > 0 && isDefined(sourceData[0][0]['label'])) {
					labelKey = 'label';
				}
		...
		bindTap: function (e) { // 点击行为
			if (this.data.disabled && this.data.disabledHint) {
				showModal(this.data.disabledHint, '提示', null, '知道了');
			}
		},
