  ...
  * 生成一个特定范围内的随机数
  */
export const genRandomNum = (min, max) => (Math.random() * (max - min + 1) | 0) + min;
...
export const genRandomString = len => {
  ...
  * 生成一个随机的字母字符串 
  */
export const genRandomAlpha = len => {
 ...
 // 拆分一维数组为二维数组
export function spArr(arr, size) {
  ...
  * @param {*} sp 
  */
export function str2Arr(str, sp = ',') {
...
export function getArrByKey(arr, key) {
  ...
  * @returns []
  */
export function getArrByKeyMulti(arr, keys = []) {
 	if (!Array.isArray(arr)) return;
  ...
  * @returns object {}
  */
export function getDataByKey(arr, key, val) {
  ...
  * @param {*} len 截取长度 -1不截取
  */
export function fmtText(content, len = -1) {
...
export function fmtMoney(s, dot = '') {
  ...
  */
export function arr2ObjectArr(arr, key1, key2, key3) {
 	let ret = [];
 	for (let k in arr) {
 		let obj = {};
  ...
  */
export function objArrSortAsc(property) {
 	return function (a, b) {
  ...
  * 用法 arr.sort(compare('age'))
  */
export function objArrSortDesc(property) {
 	return function (a, b) {
 		var value1 = a[property];
 		var value2 = b[property];
 		if (value1 < value2)
  ...
  */
export function arrAddDel(arr, data, sort = 'asc') {
 	if (!arr) return arr;
...
export function objArrMerge(arr1, arr2) {
	 for (let k in arr1) {}
}
...
export function padRight(str, len, charStr) {
 ...
 // 选项表单处理
export function getSelectOptions(str) {
 	if (!str)
 		return [];
...
export function arraySwap(arr, index1, index2) {
 	arr[index1] = arr.splice(index2, 1, arr[index1])[0];
 	return arr;
}

 // 数组置顶
export function arrayTop(arr, idx) {
 ...
 // 数组置底
export function arrayBottom(arr, idx) {
 	let node = arr.splice(idx, 1)[0];
 	arr.push(node);
 	return arr;
}
  ...
  * @param {*} obj  插入对象{}
  */
export function insertObjArrByKey(arr, key, val, obj) {
 	if (!arr) return arr;
  ...
  * @param {*} val 
  */
export function getValFromArr(arr, key = 'val', val = '') {
 	if (!Array.isArray(arr)) return null;
 	for (let k in arr) {
 		if (arr[k][key] == val)
 			return arr[k];
