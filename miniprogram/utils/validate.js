...
export function isStrAndArrNull(value) {
 	if (value === null || value === undefined) return true;
...
export function isRealNull(value) {
 	if (value === null || value === undefined) return true;
...
export function getDataType(value) {
 ...
 // 是否必填
export function checkRequired(value, desc = '') {
 ...
 // 校验字符/数组长度，校验数字大小
export function checkMin(value, min, desc = '') {
 ...
 // 校验字符/数组长度，校验数字大小
export function checkMax(value, max, desc = '') {
 	if (isStrAndArrNull(value)) return;
 ...
 // 校验字符/数组长度 
export function checkLen(value, len, desc = '') {
 	if (isStrAndArrNull(value)) return;
...
export function checkMobile(value, desc = '') {
...
export function checkInt(value, desc = '') {
 	if (isNull(value)) return;
...
export function checkDigit(value, desc = '') {
 	if (isNull(value)) return;
...
export function checkLetter(value, desc = '') {
 	if (isNull(value)) return;
...
export function checkMoney(value, desc = '') {
 	if (isNull(value)) return;
...
export function checkLetterNum(value, desc = '') {
 	if (isNull(value)) return;
...
export function checkId(value, desc = '', min = 1, max = 32) {
 	if (isNull(value)) return;
 ...
 //  邮箱
export function checkEmail(value, desc = '') {
 	if (isNull(value)) return;
...
export function checkDate(value, desc = '') {
 	if (isNull(value)) return;
 ...
 // 年份，形如 (yyyy 2008)
export function checkYear(value, desc = '') {
 	if (isNull(value)) return;
...
export function checkYearMonth(value, desc = '') {
 	if (isNull(value)) return;
 ...
 // 短时间(时分秒)，形如 (13:04:06)
export function checkTime(value, desc = '') {
 	if (isNull(value)) return;
...
export function checkHourMinute(value, desc = '') {
 	if (isNull(value)) return;
...
export function checkDatimeTime(value, desc = '') {
 	if (isNull(value)) return;
...
export function checkArray(value, desc = '') {
 	if (!Array.isArray(value))
 		return desc + '填写错误arr';
}

export function checkObject(value, desc = '') {
 	if (value.constructor != Object)
 		return desc + '填写错误obj';
 }

 export function checkBoolean(value, desc = '') {
 	if (value.constructor != Boolean)
 		return desc + '填写错误bool';
}

 // 枚举 ref=1,2,3,4格式
export function checkIn(value, ref, desc = '') {
 	if (isNull(value)) return;
...
export function checkIds(value, desc) {}

export function checkString(value, desc) {
 	if (value.constructor != String)
 		return desc + '填写错误';
}


export function check(data, rules) {
 	let returnData = {};
 	for (let k in rules) {
 		...
 		switch (dataType) {
 			case 'Array': {
				if (defVal !== undefined) {
 					try {
 						...
 						if (getDataType(defVal) != Array)
 							return _showError(desc + '默认值数组格式错误', formName);
 					} catch (ex) {
 						return _showError(desc + '默认值数组格式错误', formName);
 					}
 				}
				...
				if (val !== undefined && getDataType(val) != Array)
 					return _showError(desc + '数组格式错误', formName);
 			...
 			case 'Object': {
				if (defVal !== undefined) {
 					try {
 						...
 						if (getDataType(defVal) != Object)
 							return _showError(desc + '默认值对象格式错误', formName);
 					} catch (ex) {
 						return _showError(desc + '默认值对象格式错误', formName);
 					}
 				}
				...
				if (val !== undefined && getDataType(val) != Object)
 					return _showError(desc + '对象格式错误', formName);
 			...
 			case 'Boolean': {
				if (defVal !== undefined) {
 					try {
 						...
 						if (getDataType(defVal) != Boolean)
 							return _showError(desc + '默认值布尔格式错误', formName);
 					} catch (ex) {
				...
				if (val !== undefined && getDataType(val) != Boolean)
 					return _showError(desc + '布尔格式错误', formName);

 				break;
 			}
 			case 'Number': {
 				if (checkDigit(defVal, desc + '默认值'))
 					return _showError(desc + '默认值格式错误', formName);
				...
				if (val === undefined) break;
 				...
 				if (val === '') //数字不能为空
 					return _showError(desc + '不能为空', formName);

 				let dataType = getDataType(val);
 				if (dataType == Object || dataType == Boolean || dataType == Array)
 					return _showError(desc + '必须为数字格式', formName);

 				// 数字格式校验
 				let result = checkDigit(val, desc);
 				if (result) return _showError(result, formName);
 			...
 			case 'String': {
 				let dataType = getDataType(val);
 				if (dataType == Object || dataType == Boolean || dataType == Array)
 					return _showError(desc + '必须为字符串格式', formName);
