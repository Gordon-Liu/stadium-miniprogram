...
/** 时间格式化为年月日点分 */
export function fmtDateCHN(date, fmt = 'Y-M-D') {
	if (!date) return '';
...
export function timestame2Ago(dateTimeStamp, fmt = 'Y-M-D', diff = 0) { //dateTimeStamp是一个时间毫秒，注意时间戳是秒的形式，在这个毫秒的基础上除以1000，就是十位数的时间戳。13位数的都是时间毫秒。
...
// 获取某天0点
export function getDayFirstTimestamp(timestamp) {
 ...
 * @param {*} birth 
 */
export function getAge(birth, isMonth = false) {
	var returnAge = '';
	var mouthAge = '';
	var arr = birth.split('-');
	var birthYear = arr[0];
...
/** 获取某天所在某月第一天时间戳 */
export function getMonthFirstTimestamp(timestamp) {
...
/** 获取某天所在某月最后一天时间戳 */
export function getMonthLastTimestamp(timestamp) {
	let inDate = new Date(timestamp);
	let year = inDate.getFullYear();
	let month = inDate.getMonth();
...
// 取得分钟时间戳
export function getNowMinTimestamp() {
...
// 获取当前日期所在周一 输入和返回格式=yyyy-mm-dd
export function getLastOfWeek(date) {
	let now = new Date(date);
	let nowTime = now.getTime();
	let day = now.getDay();
	if (day == 0) day = 7;
 ...
 * @param {*} flag 1=正 -1=负
 */
export function getTimeLeft(datetimeTo, flag = 1) {
