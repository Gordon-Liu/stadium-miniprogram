...
export function leapMonthDays(ly) {
	if (hasLeapMonth(ly)) {
		// 获取16进制的第1位（0x除外）
...
export function lunarYearDays(ly) {
	let totalDays = 0;
...
// 获取农历每个月的天数
// 参数需传入16进制数值
export function lunarYearMonths(ly) {
	let monthArr = [];
 ...
 * @param {*} sd 
 */
export function getSolarTerm(sy, sm, sd) {
	sm -= 1;
	let solarTermStr = "";
